...
compileAndRun() {
  ...
  if [[ $instrset -lt 3 ]] ; then
    isetoption="-march=armv8-a+fp+simd+crypto+crc"
    ...
    isetoption="-march=armv8-a+fp+simd+crypto+crc" #-msse4.2
...
setCompiler() {
  ...
  if [ $compiler -eq 1 ] ; then
    # Gnu compiler
    options="-std=c++17 -O3 -fno-trapping-math -o$exefilename"
    ...
    # Intel compiler for Linux
    options="-std=c++17 -O3 -fno-trapping-math -fp-model precise -o$exefilename"    
  elif [ $compiler -eq 10 ] ; then
...
getInstructionSet() {
  ...
  if [ $compiler -eq 1 ] ; then
      # Gnu compiler
      eval g++ -march=armv8-a+fp+simd+crypto+crc -std=c++17 -I$include get_instruction_set.cpp -oiset$exefilename
      
  elif [ $compiler -eq 2 ] ; then
      # Clang compiler
      eval clang -march=armv8-a+fp+simd+crypto+crc -std=c++17 -I$include get_instruction_set.cpp -oiset$exefilename
